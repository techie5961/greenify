    ...
    <section class="section1 p-10 w-full">
        <div class="card">
            <div class="row align-center g-5">
                ...
                <strong class="desc text-gradient">Edit {{ $product->name }}</strong>
            </div>
            
            <form enctype="multipart/form-data" class="column" action="{{ url()->to('admins/post/edit/product/process') }}" method="POST" onsubmit="PostRequest(event,this,MyFunc.CallBack)">
             <input type="hidden" value="{{ $product->id }}" name="id" class="input">
                <input type="hidden" name="_token" class="input" value="{{ csrf_token() }}">
                <label style="background-image: url('{{ asset('products/'.$product->photo.'') }}')" for="photo" class="br-20 max-w-200 pointer clip-20 column justify-center c-primary x-auto w-half border-dashed-2 perfect-square  bg-image">
                    <div style="display:none;" class="column content p-10 text-center c no-select justify-center g-5">
                ...
                <button class="post"><div class="content">Save Changes</div></button>
            </form>
        </div>
    </section>
@endsection
